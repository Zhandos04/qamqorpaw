...
const DogAdoptionPage = () => {
  ...
  const fetchDogs = useCallback(async (pageNum = 1, searchQuery = search, currentFilters = filters) => {
    setLoading(true);
    try {
      const queryParams = {
        pet_type: 'dog',
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const [breedsResponse, citiesResponse] = await Promise.all([
          petsApi.getAllBreeds({ pet_type: 'dog' }),
  ...
  const clearFilters = () => {
    ...
    setPage(1);
    fetchDogs(1, search, resetFilters);
  };
  
  const handleFavoriteToggle = (petId) => {
    setDogs(dogs.map(dog => 
      dog.id === petId ? { ...dog, is_favorite: !dog.is_favorite } : dog
    ));
  };
